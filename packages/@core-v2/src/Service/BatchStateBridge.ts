...
import { broadcastDomainEvent, EventBroadcastHub } from "../Runtime/EventBroadcastRouter.js"
...
const makeBatchStateBridge = Effect.gen(function*() {
  ...
  const eventBroadcastHub = yield* EventBroadcastHub
  ...
  const fiber = yield* Stream.fromQueue(subscription).pipe(
    ...
    Effect.catchAll((error) =>
      Effect.logError("BatchStateBridge error", { error: String(error) })
    ),
    Effect.fork
  )
